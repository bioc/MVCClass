...
\abstract{ In this paper we consider a generalization of the model
...
\Rpackage{MVC} available at \url{http://www.bioconductor.org}. }
...
for, what we believe to be, easy extension to linked data sets.
...
of the initial data set. This leads to complicatins in the usual MVC
...
the hands of the user, should they want it.
...
sorts of interactions we would like to support we here provide a short
...
located on, what sorts of activities they are engaged in and in many
...
for the heatmap view the basic element is generally the gene. 
...
more based on component software constructed within an object oriented
...
system. Therefore, we built our software on top the \Rpackage{RGtk} of
...
sophisticate developer who might want to take our basic infrastructure
and create an interactive system, say for the interactive analysis of
time series data, using our infrastructure. Hence, all commands are
...
linked data sets are given. Our model is typically a statistical data
matrix which represents data on a number of variables, for each of a
...
created as views, where on displays sample one versus sample two and
the second displays sample three versus sample four, then the points
...
By \textit{inked data sets} we mean that the data have more than one
...
set 1 tells its view to update itself.
...
structure for the three types of models we have implemented, on based
...
must respond to a certain subset of events so we have implemented this
class by first creating a single, virtual class which the specialized
views extend.  As for our MVC classes, this allows us to define
...
frame. Other views we have implemented include: scatterplot
(\Rclass{sPlotView}), graph (\Rclass{graphView}), and heatmaps
(\Rclass{heatmapView}).
...
\Rfunction{clickEvent} method that is invoked.  Using the information
that is passed to it, the method converts the pixel coordinates into
user coordinates and then finds the closest object in the plot.
...
actions taken depend on the view, for example for a spread sheet view
the method may select or unselect a row on the spreadsheeet while for
a scatterplot view the method may change the appearance of a point on
...
functions only allow the user create a scatterplot or a spreadsheet
...
preprocessing functions.  The preprocessing functions take no
...
user to choose what the new color is.
...
commandline functions.  Note that not all callback functions make
sense for each event so the controller environment also stores a list,
for each even,t of appropriate instances that can be linked to that
event.
...
message classes for these elements.  These messages can be sent to
either a model, or a view and their effect is quite different. 
...
in Figure~\ref{Fig:MPwithin}.  In this example, the response to a user
...
in the scatter plot, which is shown as Step 1 (the circled 1 on the
...
model, which updates itself accordingly, ie. by changing the color
...
this model.  In response to this message, the view updates itself in
Step 4 and now the point is colored red on the view to reflect the
change in the model. Any other view that contains this point should
also be altered to reflect the new color.
...
experimental microarray data are linked to the GO terms.  Thus, to
pass information between these two data sets requires that these
...
model1's model changes, then this information first gets passed to
model2.1 and model2.2 and these two \Robject{MVCs} may make changes to
their models based on this information.  Then model2.1 passes this
information to model3.1, model3.2, and model3.3 based on the changes
that model2.1 made to its model.  Thus, information passing between
model1 and model3.1, for instance, only occurs through model2.1.  So
...
this particular child model.  Thus, the message sent may be specific
to the child. 

In Section~\ref{Ssec:OneMess} we was noted that
...
%%FIXME: this belongs in the introductin. Here you should say
